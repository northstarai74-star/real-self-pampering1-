...
import { z } from "zod";
import { BadgeCheck, Search, AlertCircle, Loader2 } from "lucide-react";
import { Reveal } from "@/components/Reveal";
import { verifyCertificatePublic, type VerificationResult } from "@/lib/certificates-api";
...
export function CertificateCheck() {
  ...
  const onSubmit = async (event: React.FormEvent) => {
    ...
    setChecking(true);
    try {
      const outcome = await verifyCertificatePublic({ data: parsed.data });
      setResult(outcome);
    } catch {
      setResult(null);
      setErrors({ number: "Could not reach the certificate register. Please try again." });
    } finally {
      setChecking(false);
    }
  };
        ...
        <Reveal delay={120}>
          <form onSubmit={onSubmit} noValidate className="border border-border bg-background p-6 md:p-10">
            ...
            <button
              type="submit"
              disabled={checking}
              className="label-xs mt-8 inline-flex items-center gap-3 border border-plum bg-plum px-8 py-4 text-primary-foreground transition-colors duration-300 hover:bg-ink disabled:cursor-not-allowed disabled:opacity-60"
            >
              {checking ? (
